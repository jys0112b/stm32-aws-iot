...
//	{
		if(mqttClient.isconnected)
		{
			...
			MQTTPublish(&mqttClient, "sdkTest/sub", &message); //publish a message
		}
//		osDelay(1000);
//	}
}
...
int MqttConnectBroker()
{
	...
	data.willFlag = 0;
	data.MQTTVersion = 3;
	data.clientID.cstring = ""; // 사물 세부정보 - 사물 ARN 에서 thing/  다음에 오는 이름으로 설정했음.
	data.username.cstring = ""; // 사물 세부정보 - 사물 ARN 에서 thing/  다음에 오는 이름으로 설정했음.
	data.password.cstring = ""; // password 는 공란으로 두었음.
	...
	//ret = MQTTSubscribe(&mqttClient, "sdkTest/sub", QOS0, MqttMessageArrived);
	MqttClientPubTask(NULL);
	if(ret != MQTT_SUCCESS)
	{
